...
9. `NA->名称`为占位线指定精确名称，例如`NA->ready_test_process`生成`wire ... ready_test_process`并连接真实模块端口。`NA->0`或`NA->1`表示常量目标；对于 TOP output，生成对应`assign`且同一行的子模块 output 开路，避免把常量接到输出端。普通`NA->1`会按目标端口全部 packed 维度生成全 1 复制式，例如`[A][B]`信号生成`{A*B{1'b1}}`，而不是用标量`1`导致只有最低位为 1；带显式尺寸的其他 Verilog 常量保持原样。未连接、单端和 NA 占位类诊断均为 info，不会使`--strict`失败。
